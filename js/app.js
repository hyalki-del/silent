 ...
 * CS Senior Architecture: Monolithic State Machine & Localized Controller
 * ==========================================================================
 */
...
function initTaglineCarousel() {
    ...
    function cycleTagline() {
        ...
        if (isLogoStep) {
            ...
            const textIndex = Math.floor(carouselStepIndex / 2) % taglines.length;
            spot.innerHTML = taglines[textIndex];
        }
...
function getCurrentCardOrder() {
    const container = document.getElementById('appContainer');
    return container ? Array.from(container.querySelectorAll('.theme-card')).map(card => card.getAttribute('data-card-id')).filter(Boolean) : [];
}

function applyCardOrder(orderArray) {
    if (!Array.isArray(orderArray) || orderArray.length === 0) return;
    const container = document.getElementById('appContainer');
    if (container) orderArray.forEach(id => { const card = container.querySelector(`[data-card-id="${id}"]`); if (card) container.appendChild(card); });
}
...
async function callBackend(action, payload = {}) {
    try {
        ...
        const response = await fetch(sheetUrl, { method: 'POST', body: JSON.stringify({ action, tab: currentTab, pin: currentPin, ...payload }) });
...
async function loadGoogleSheetsArchive() {
    ...
    if (!select) return;
    if (select.options.length <= 1) select.innerHTML = `<option value="">-- Reading Google Sheets... --</option>`;
    try {
        const sheetUrl = await getConfig();
        if (!sheetUrl) return;
        const res = await fetch(sheetUrl);
        const rawData = await res.json();
        let ledgers = Array.isArray(rawData) ? rawData : (rawData.archives || rawData.sheets || rawData.ledgers || Object.keys(rawData));
    ...
    } catch (error) { select.innerHTML = `<option value="">-- Error loading archives --</option>`; }
}

function switchModalTab(tabMode) {
    const createSec = document.getElementById('createSection'), recallSec = document.getElementById('recallSection');
    const tabCreateBtn = document.getElementById('tabCreateBtn'), tabRecallBtn = document.getElementById('tabRecallBtn');
    if (!createSec || !recallSec) return;
    if (tabMode === 'create') {
        createSec.classList.remove('hidden'); recallSec.classList.add('hidden');
        ...
        createSec.classList.add('hidden'); recallSec.classList.remove('hidden');
        ...
        const select = document.getElementById('archiveSelect');
        if (select && (select.options.length <= 1 || select.value === "")) loadGoogleSheetsArchive();
    }
}
...
async function createNewLedger() {
    ...
    if (!nameVal || pinVal.length !== 4) { alert("Please enter a valid ledger name and a 4-digit PIN."); return; }
    ...
    if (res && res.status === "success") {
        currentTab = res.createdTab; currentPin = pinVal; ledgerData = { members: [], expenses: [] }; unsavedMembers = [];
...
function goHome() {
    ...
    document.getElementById('welcomeModal')?.classList.remove('hidden'); render(); initTaglineCarousel();
}

async function deleteActiveLedger() { if (!confirm("Delete active ledger?")) return; await callBackend('deleteLedger'); goHome(); }
...
async function saveMembers() {
    ...
    if (res && res.status === "success") { unsavedMembers = []; render(); }
}
...
function cancelEditExpense() { editingExpenseId = null; resetExpenseForm(); render(); }

function resetExpenseForm() {
    const descInput = document.getElementById('expenseDesc'), amountInput = document.getElementById('expenseAmount');
...
async function updateExpense() {
    ...
    if (!date || !desc || isNaN(amount) || amount <= 0 || !paidBy) return;
    ...
    editingExpenseId = null; resetExpenseForm(); render();
...
function calculateSettlement() {
    ...
    Object.keys(balances).forEach(lowerKey => {
        const bal = balances[lowerKey], displayName = lowerMap[lowerKey] || lowerKey;
...
function copySettlementSummary() {
    ...
    navigator.clipboard.writeText(summaryText).then(() => alert("Copied settlement summary!"));
}
...
function generateLedgerReport() {
    ...
    else report += `${t.allSettled || 'All balances are currently settled!'}\n`;
    report += `\n----------------------------------------------------\n\n`;
    ...
    } else report += `${t.noExpenses || 'No expenses recorded.'}\n`;
    report += `====================================================\n`;
...
function renderDropdowns() {
    ...
    catSelect.innerHTML = defaultCategories.map(catKey => `<option value="${catKey}">${escapeHTML(getLocalizedCategory(catKey))}</option>`).join('');
...
function renderSplitCheckboxes() {
    ...
    if (!container) return;
    const t = TRANSLATIONS[currentLang] || TRANSLATIONS['en'];
    const userSelectedValues = Array.from(document.querySelectorAll('.split-checkbox:checked')).map(cb => cb.value.toLowerCase());
        ...
        ? ledgerData.expenses.map(e => {
            ...
            <li class="p-2.5 rounded-xl border border-current/15 flex justify-between items-center bg-current/5 gap-2">
                <div class="flex-1 min-w-0">
                    ...
                    <div class="text-[10px] opacity-70">${t.paidByText || 'Paid by'} <span class="font-bold">${escapeHTML(canonicalPayer)}</span> • ${displayDate} • ${t.splitText || 'Split:'} ${escapeHTML(displaySplits)}</div>
                </div>
                <div class="flex items-center gap-2 flex-shrink-0">
                    <span class="font-extrabold text-sm">${getCurrencySymbol()}${parseFloat(e.amount).toFixed(2)}</span>
                    <button type="button" data-id="${e.id}" onclick="window.startEditExpense(this.getAttribute('data-id'))" class="theme-btn px-2.5 py-1 text-[10px] font-black uppercase tracking-wider bg-amber-300 text-slate-900 cursor-pointer hover:bg-amber-400">
                        ${t.editItemBtn || 'Edit'}
                    </button>
                </div>
            </li>
...
window.startEditExpense = startEditExpense; window.cancelEditExpense = cancelEditExpense; window.updateExpense = updateExpense;
